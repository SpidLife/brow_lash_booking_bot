...
JOURNAL_STATUSES = {
    "active": ("confirmed", "🟢 Актуальные"),
    "cancelled": ("cancelled", "❌ Отменённые"),
    "moved": ("rescheduled", "🔄 Перенесённые"),
    "all": ("all", "📚 Все статусы"),
}
JOURNAL_PAGE_SIZE = 6
...
class BeautyBot:
    ...
    def show_home(self, user_id: int, message_id: int | None = None) -> None:
        ...
        if message_id:
            ...
            sent = self.api.send(user_id, greeting, keyboard)
    ...
    def handle_message(self, message: dict[str, Any]) -> None:
        ...
        if text in {"📚 Все записи", "Все записи"}:
            if not self.is_admin(user_id):
                ...
                return
            self.db.clear_state(user_id)
            self.show_booking_journal(user_id, message_id=self.panel_message_ids.get(user_id))
            return

        if text in {"/cancel", "🏠 Главное меню", "← Назад", "↩️ Назад"}:
    ...
    def journal_period(self, period: str) -> tuple[date | None, date | None, str]:
        today = self.now().date()
        if period == "future":
            return today, None, "Все будущие даты"
        if period == "today":
            return today, today, "Сегодня"
        if period == "tomorrow":
            tomorrow = today + timedelta(days=1)
            return tomorrow, tomorrow, "Завтра"
        if period == "week":
            return today, today + timedelta(days=6), "Ближайшие 7 дней"
        if period == "two":
            return today, today + timedelta(days=13), "Ближайшие 2 недели"
        if period == "all":
            return None, None, "Все даты и история"
        if period.startswith("d") and len(period) == 9:
            selected = self.decode_date(period[1:])
            return selected, selected, pretty_date(selected)
        raise ValueError("Неизвестный период записей")

    @staticmethod
    def journal_booking_indicator(booking: Any) -> str:
    ...
    def show_booking_journal(
        ...
        if status not in JOURNAL_STATUSES or page < 0:
            raise ValueError("Неизвестный фильтр записей")
        database_status, status_title = JOURNAL_STATUSES[status]
        start_date, end_date, period_title = self.journal_period(period)
        overview = self.db.admin_booking_overview(start_date=start_date, end_date=end_date)
        count = overview["total"] if database_status == "all" else overview[database_status]
        page = min(page, max((count - 1) // JOURNAL_PAGE_SIZE, 0))
        bookings = self.db.admin_bookings(
        ...
        lines = [
            "📚 <b>Все записи</b>",
            f"🔎 Фильтр: <b>{status_title}</b>",
            f"📅 Период: <b>{escape(period_title)}</b>",
            "",
            f"🟢 Актуальных: <b>{overview['confirmed']}</b> · "
            f"❌ Отменено: <b>{overview['cancelled']}</b> · "
            f"🔄 Перенесено: <b>{overview['rescheduled']}</b>",
            f"💰 Ожидаемая выручка: <b>{overview['revenue']} {escape(self.settings.currency)}</b>",
        ]
        rows: list[list[tuple[str, str]]] = []
        ...
        if bookings:
            ...
            for booking in bookings:
                work_date = date.fromisoformat(booking["work_date"])
                if booking["work_date"] != previous_date:
                    lines.append(f"<b>{pretty_date(work_date)}</b>")
                    previous_date = str(booking["work_date"])
                indicator = self.journal_booking_indicator(booking)
                time_label = minutes_to_hhmm(int(booking["start_minutes"]))
                lines.append(
                    f"{indicator} {time_label} · {escape(booking['customer_name'])}\n"
                    f"   💅 {escape(booking['service_name'])}"
                )
                short_date = work_date.strftime("%d.%m")
                rows.append([(
                    f"{indicator} {short_date} {time_label} · {booking['customer_name']}",
                    f"ajdetail:{booking['id']}:{status}:{period}:{page}",
                )])
        else:
            lines.extend(["", "По выбранным фильтрам записей нет."])

        if count > JOURNAL_PAGE_SIZE:
            pages = (count + JOURNAL_PAGE_SIZE - 1) // JOURNAL_PAGE_SIZE
            lines.append(f"\nСтраница {page + 1}/{pages} · всего записей: {count}")
            pagination: list[tuple[str, str]] = []
            if page > 0:
                pagination.append(("‹ Назад", f"ajournal:{status}:{period}:{page - 1}"))
            if page + 1 < pages:
                pagination.append(("Вперёд ›", f"ajournal:{status}:{period}:{page + 1}"))
            if pagination:
                rows.append(pagination)

        def status_button(code: str) -> tuple[str, str]:
            marker = "• " if code == status else ""
            titles = {
                "active": "🟢 Актуальные",
                "cancelled": "❌ Отмены",
                "moved": "🔄 Переносы",
            ...
            return marker + titles[code], f"ajournal:{code}:{period}:0"

        rows.extend([
            [status_button("active"), status_button("cancelled")],
            [status_button("moved"), status_button("all")],
            [("Сегодня", f"ajournal:{status}:today:0"), ("Завтра", f"ajournal:{status}:tomorrow:0")],
            [("7 дней", f"ajournal:{status}:week:0"), ("2 недели", f"ajournal:{status}:two:0")],
            [("📅 Выбрать дату", f"ajcalendar:{status}:0")],
            [("🗓 Будущие", f"ajournal:{status}:future:0"), ("📂 Вся история", f"ajournal:{status}:all:0")],
            [("← В главное меню", "home")],
        ])
        self.show_panel(user_id, "\n".join(lines), inline(*rows), message_id)

    def show_journal_calendar(
        self,
        user_id: int,
        status: str,
        page: int = 0,
        ...
        if status not in JOURNAL_STATUSES or not -52 <= page <= 52:
            raise ValueError("Некорректная страница календаря")
        rows: list[list[tuple[str, str]]] = []
        first_day = self.now().date() + timedelta(days=page * 7)
        for offset in range(7):
            work_date = first_day + timedelta(days=offset)
            counts = self.db.admin_booking_overview(start_date=work_date, end_date=work_date)
            label = (
                f"{pretty_date(work_date)} · 🟢 {counts['confirmed']} "
                f"❌ {counts['cancelled']} 🔄 {counts['rescheduled']}"
            )
            rows.append([(label, f"ajournal:{status}:d{work_date.strftime('%Y%m%d')}:0")])
        navigation: list[tuple[str, str]] = []
        if page > -52:
            navigation.append(("‹ Предыдущая неделя", f"ajcalendar:{status}:{page - 1}"))
        if page < 52:
            navigation.append(("Следующая неделя ›", f"ajcalendar:{status}:{page + 1}"))
        if navigation:
            rows.append(navigation)
        rows.append([("← Ко всем записям", f"ajournal:{status}:future:0")])
        text = (
            "📅 <b>Выбери дату записей</b>\n\n"
            "Под каждым днём показано количество актуальных, отменённых и перенесённых записей."
        )
        self.show_panel(user_id, text, inline(*rows), message_id)

    def show_journal_booking(
        self,
        user_id: int,
        booking_id: int,
        status: str,
        ...
        if status not in JOURNAL_STATUSES or page < 0:
            raise ValueError("Некорректный фильтр записи")
        self.journal_period(period)
        booking = self.db.booking(booking_id)
        if not booking:
            self.show_booking_journal(user_id, status, period, page, message_id)
            return
        state = {
            "confirmed": self.attendance_label(booking),
            "cancelled": "❌ Отменена",
            "rescheduled": "🔄 Перенесена на другое время",
        }.get(str(booking["status"]), "Неизвестный статус")
        username = f"@{escape(booking['username'])}" if booking["username"] else "не указан"
        ...
        rows.append([("← Ко всем записям", f"ajournal:{status}:{period}:{page}")])
        self.show_panel(user_id, text, inline(*rows), message_id)
    ...
    def handle_callback(self, query: dict[str, Any]) -> None:
        ...
        if message_id:
            self.panel_message_ids[user_id] = int(message_id)
    ...
    def handle_admin_callback(self, user_id: int, parts: list[str], message_id: int | None) -> None:
        ...
        elif action == "ajournal":
            self.show_booking_journal(user_id, parts[1], parts[2], int(parts[3]), message_id)
        elif action == "ajcalendar":
            self.show_journal_calendar(user_id, parts[1], int(parts[2]), message_id)
        elif action == "ajdetail":
            self.show_journal_booking(user_id, int(parts[1]), parts[2], parts[3], int(parts[4]), message_id)
        elif action == "abook":
